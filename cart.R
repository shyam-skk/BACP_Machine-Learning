...
prop.table(table(CTDF.dev$TARGET))
table(CTDF.holdout$TARGET)
prop.table(table(CTDF.holdout$TARGET))
...
m3 <- rpart(formula = TARGET ~ ., 
            data = CTDF.dev[,-c(1,11)], method = "class", 
            control = r.ctrl)
m3
